  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        try {
          const clanRes = await adminFetch<any>('/clans');
          const raw = Array.isArray(clanRes)
            ? clanRes
            : Array.isArray(clanRes?.clans)
              ? clanRes.clans
              : [];
          setClans(raw);
        } catch {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
            Tree Diagnostics & Health
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Monitor tree engine health, run diagnostics, and view analytics
          </p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            {[
              {
                label: 'Total Members',
                value: health.totals?.members || 0,
                color: 'text-emerald-600 dark:text-emerald-400',
              },
              {
                label: 'Total Families',
                value: health.totals?.families || 0,
                color: 'text-blue-600 dark:text-blue-400',
              },
              {
                label: 'Total Clans',
                value: health.totals?.clans || 0,
                color: 'text-violet-600 dark:text-violet-400',
              },
              {
                label: 'Tree Views',
                value: health.totals?.views || 0,
                color: 'text-amber-600 dark:text-amber-400',
              },
              {
                label: 'Layout Caches',
                value: health.totals?.layoutCaches || 0,
                color: 'text-cyan-600 dark:text-cyan-400',
              },
              {
                label: 'Bookmarks',
                value: health.totals?.bookmarks || 0,
              ...
              <div className="flex items-center gap-4">
                <div
                  className={`flex h-16 w-16 items-center justify-center rounded-full text-2xl font-bold ${
                    health.health?.healthStatus === 'Excellent'
                      ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400'
                      : health.health?.healthStatus === 'Good'
                        ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'
                        : health.health?.healthStatus === 'Fair'
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    {health.health?.brokenRelationships || 0} broken relationships
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-900">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-slate-900 dark:text-white">
                    {perf?.avgMembersPerFamily || 0}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500 dark:text-slate-400">Virtual Rendering</span>
                  <span
                    className={`font-medium ${perf?.virtualRenderingReady ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400'}`}
                  >
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-900">
            ...
            <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">
              Check for broken relationships, orphan nodes, and health issues
            </p>
            <div className="flex flex-wrap items-end gap-3">
              <div>
                <label className="mb-1 block text-xs text-slate-400 dark:text-slate-500">
                  Entity Type
                </label>
                <select
                  value={diagEntityType}
                  onChange={(e) => setDiagEntityType(e.target.value)}
              ...
              <div className="flex-1 min-w-[200px]">
                ...
                <select
                  value={diagEntityId}
                  onChange={(e) => setDiagEntityId(e.target.value)}
              ...
              <button
                ...
                disabled={!diagEntityId || diagLoading}
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-900">
                  ...
                  <div className="space-y-1">
                    {diagnostics.generationStats?.membersPerGeneration &&
                      Object.entries(diagnostics.generationStats.membersPerGeneration).map(
                        ([gen, count]: [string, any]) => (
                          <div key={gen} className="flex items-center gap-2">
                            <span className="w-8 text-xs text-slate-400 dark:text-slate-500">
                              Gen {gen}
                            </span>
                            <div
                              className="h-4 rounded bg-emerald-100 dark:bg-emerald-900/30"
                              style={{
                                width: `${Math.max(10, (count / diagnostics.totalNodes) * 100)}%`,
                              }}
                            />
                  ...
                  <div className="space-y-1">
                    {diagnostics.details.brokenRelationships.map((br: any, i: number) => (
                      <div key={i} className="text-xs text-red-600 dark:text-red-400">
                        Edge {br.edgeId}: {br.reason} (from: {br.from?.slice(0, 8)}... to:{' '}
                        {br.to?.slice(0, 8)}...)
                      </div>
                    ))}
                  </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-900">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div>
                <h4 className="mb-1 text-xs font-medium text-slate-400 dark:text-slate-500">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {['VERTICAL', 'HORIZONTAL', 'COMPACT', 'BALANCED'].map((l) => (
                    <span
                      key={l}
                      className="rounded bg-blue-100 px-2 py-0.5 text-[10px] font-medium text-blue-700 dark:bg-blue-900/30 dark:text-blue-400"
                    >
                      {l}
                    </span>
                  ))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {['Gender', 'Generation', 'Entity'].map((c) => (
                    <span
                      key={c}
                      className="rounded bg-violet-100 px-2 py-0.5 text-[10px] font-medium text-violet-700 dark:bg-violet-900/30 dark:text-violet-400"
                    >
                      {c}
                    </span>
                  ))}
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {[
                    ...
                    <span
                      key={f}
                      className="rounded bg-amber-100 px-2 py-0.5 text-[10px] font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
                    >
                      {f}
                    </span>
                  ))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {['Public Views', 'SEO Metadata', 'Canonical URLs', 'OpenGraph'].map((f) => (
                    <span
                      key={f}
                      className="rounded bg-cyan-100 px-2 py-0.5 text-[10px] font-medium text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400"
                    >
                      {f}
                    </span>
                  ))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {['Bookmarks', 'Search History', 'View History', 'Layout Cache', 'Bookmarks'].map(
                    (f) => (
                      <span
                        key={f}
                        className="rounded bg-pink-100 px-2 py-0.5 text-[10px] font-medium text-pink-700 dark:bg-pink-900/30 dark:text-pink-400"
                      >
                        {f}
                      </span>
                    ),
