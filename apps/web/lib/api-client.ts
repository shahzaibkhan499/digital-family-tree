    ...
    if (!refreshPromise) {
      refreshPromise = (async () => {
        try {
          ...
          const refreshTimeoutId = setTimeout(() => refreshController.abort(), API_TIMEOUT);
...
export const api = {
  auth: {
    register: (data: { name: string; email: string; password: string }) =>
      request<{ user: any; access_token: string; refresh_token?: string }>('/auth/register', { method: 'POST', body: data }),
    login: (data: { email: string; password: string }) =>
      request<{ user: any; access_token: string; refresh_token?: string }>('/auth/login', { method: 'POST', body: data }),
  ...
  profile: {
    ...
    update: (data: any) =>
      request<any>('/profile', { method: 'PATCH', body: data }),
    updatePrivacy: (data: { privacySettings: string }) =>
      request<any>('/profile/privacy', { method: 'PATCH', body: data }),
    getCompletion: () => request<{ percentage: number; missingFields: string[] }>('/profile/completion'),
    getSessions: () => request<any[]>('/profile/sessions'),
    claimUsername: (data: { username: string }) =>
      request<{ id: string; username: string; profileSlug: string }>('/profile/username', { method: 'POST', body: data }),
    getFieldPrivacy: () => request<any>('/profile/privacy-fields'),
    updateFieldPrivacy: (fields: { fieldName: string; visibility: string }[]) =>
      request<any>('/profile/privacy-fields', { method: 'PATCH', body: JSON.stringify({ fields }) }),
  ...
  families: {
    ...
    delete: (id: string) =>
      request<any>(`/families/${id}`, { method: 'DELETE' }),
    stats: () =>
      request<{ totalFamilies: number; totalMembers: number; totalRelationships: number }>('/families/stats'),
  ...
  members: {
    list: (familyId: string) =>
      request<any[]>(`/families/${familyId}/members`),
    ...
    checkDuplicate: (familyId: string, data: { firstName: string; lastName: string; birthDate?: string }) =>
      request<{ duplicate: boolean; member: any }>(`/families/${familyId}/members/check-duplicate`, { method: 'POST', body: data }),
    checkGlobalDuplicate: (data: any) =>
      request<{ hasDuplicates: boolean; duplicates: any[] }>('/families/any/members/check-global-duplicate', { method: 'POST', body: data }),
  },
  relationships: {
    list: (familyId: string) =>
      request<any[]>(`/families/${familyId}/relationships`),
    create: (data: { fromMemberId: string; toMemberId: string; type: string }) =>
      request<any>('/relationships', { method: 'POST', body: data }),
    remove: (id: string) =>
      request<any>(`/relationships/${id}`, { method: 'DELETE' }),
  },
  invitations: {
    ...
    checkUser: (email: string) => request<{ exists: boolean; user: any }>(`/invitations/check-user?email=${encodeURIComponent(email)}`),
  },
  discovery: {
  ...
  duplicates: {
    ...
    review: (id: string, action: string) => request(`/duplicates/${id}/review`, { method: 'PATCH', body: JSON.stringify({ action }) }),
    byFamily: (familyId: string) => request(`/duplicates/family/${familyId}`),
  },
  smartInvite: {
    search: (familyId: string, query: string) => request(`/families/${familyId}/members/smart-invite?query=${encodeURIComponent(query)}`),
  },
  search: {
    global: (query: string, options?: { page?: number; limit?: number; type?: string }) => {
      ...
      return request<{ users: any[]; members: any[]; families: any[]; total: number }>(`/search?${params.toString()}`);
    },
  },
  merge: {
    ...
    preview: (sourceMemberId: string, targetMemberId: string) => request<any>(`/merge/preview?sourceMemberId=${sourceMemberId}&targetMemberId=${targetMemberId}`),
    execute: (data: { sourceMemberId: string; targetMemberId: string; strategy: string }) => request<any>('/merge/execute', { method: 'POST', body: JSON.stringify(data) }),
  ...
  notifications: {
    list: (options?: { page?: number; limit?: number; type?: string; category?: string; isRead?: boolean; isArchived?: boolean; search?: string; dateFrom?: string; dateTo?: string }) => {
      ...
      return request<{ notifications: any[]; total: number; page: number; limit: number; totalPages: number }>(`/notifications?${params.toString()}`);
    },
    ...
    preferences: {
      ...
      update: (data: Record<string, boolean>) => request<any>('/notifications/preferences', { method: 'PATCH', body: data }),
    },
    stats: () => request<any>('/notifications/stats'),
  },
  activities: {
    list: (options?: { page?: number; limit?: number; search?: string; eventType?: string; visibility?: string; familyId?: string; userId?: string; dateFrom?: string; dateTo?: string; sortBy?: string; sortOrder?: string }) => {
      const params = new URLSearchParams();
      if (options?.page) params.set('page', String(options.page));
      if (options?.limit) params.set('limit', String(options.limit));
      if (options?.search) params.set('search', options.search);
      ...
      return request<{ activities: any[]; total: number; page: number; limit: number; totalPages: number }>(`/activities?${params.toString()}`);
    },
    mine: (options?: { page?: number; limit?: number; eventType?: string }) => {
      ...
      if (options?.limit) params.set('limit', String(options.limit));
      if (options?.eventType) params.set('eventType', options.eventType);
      return request<{ activities: any[]; total: number; page: number; limit: number; totalPages: number }>(`/activities/me?${params.toString()}`);
    },
    get: (id: string) => request<any>(`/activities/${id}`),
    byFamily: (familyId: string, options?: { page?: number; limit?: number; eventType?: string; dateFrom?: string; dateTo?: string }) => {
      const params = new URLSearchParams();
      if (options?.page) params.set('page', String(options.page));
      if (options?.limit) params.set('limit', String(options.limit));
      if (options?.eventType) params.set('eventType', options.eventType);
      if (options?.dateFrom) params.set('dateFrom', options.dateFrom);
      if (options?.dateTo) params.set('dateTo', options.dateTo);
      return request<{ activities: any[]; total: number; page: number; limit: number; totalPages: number }>(`/activities/family/${familyId}?${params.toString()}`);
    },
    byUser: (userId: string, options?: { page?: number; limit?: number; eventType?: string }) => {
      ...
      if (options?.limit) params.set('limit', String(options.limit));
      if (options?.eventType) params.set('eventType', options.eventType);
      return request<{ activities: any[]; total: number; page: number; limit: number; totalPages: number }>(`/activities/user/${userId}?${params.toString()}`);
    },
    ...
    toggleReaction: (id: string, type?: string) =>
      request<any>(`/activities/${id}/reactions`, { method: 'POST', body: { type: type || 'LIKE' } }),
    delete: (id: string) =>
      request<any>(`/activities/${id}`, { method: 'DELETE' }),
  },
  memories: {
    create: (data: any) =>
      request<any>('/memories', { method: 'POST', body: data }),
    list: (options?: { page?: number; limit?: number; search?: string; familyId?: string; memberId?: string; tag?: string; location?: string; dateFrom?: string; dateTo?: string; sortBy?: string; sortOrder?: string }) => {
      const params = new URLSearchParams();
      if (options?.page) params.set('page', String(options.page));
      if (options?.limit) params.set('limit', String(options.limit));
      if (options?.search) params.set('search', options.search);
      ...
      if (options?.sortBy) params.set('sortBy', options.sortBy);
      if (options?.sortOrder) params.set('sortOrder', options.sortOrder);
      return request<{ memories: any[]; total: number; page: number; limit: number; totalPages: number }>(`/memories?${params.toString()}`);
    },
    ...
    byFamily: (familyId: string, options?: { page?: number; limit?: number; sortBy?: string; sortOrder?: string }) => {
      const params = new URLSearchParams();
      if (options?.page) params.set('page', String(options.page));
      if (options?.limit) params.set('limit', String(options.limit));
      if (options?.sortBy) params.set('sortBy', options.sortBy);
      if (options?.sortOrder) params.set('sortOrder', options.sortOrder);
      return request<{ memories: any[]; total: number; page: number; limit: number; totalPages: number }>(`/memories/family/${familyId}?${params.toString()}`);
    },
    byMember: (memberId: string, options?: { page?: number; limit?: number }) => {
      const params = new URLSearchParams();
      if (options?.page) params.set('page', String(options.page));
      if (options?.limit) params.set('limit', String(options.limit));
      return request<{ memories: any[]; total: number; page: number; limit: number; totalPages: number }>(`/memories/member/${memberId}?${params.toString()}`);
    },
    addComment: (id: string, content: string) =>
  ...
  timeline: {
    create: (data: any) =>
      request<any>('/timeline', { method: 'POST', body: data }),
    list: (options?: { page?: number; limit?: number; familyId?: string; memberId?: string; eventType?: string; category?: string; status?: string; visibility?: string; dateFrom?: string; dateTo?: string; cursor?: string; search?: string; sortBy?: string; sortOrder?: string }) => {
      const params = new URLSearchParams();
      if (options?.page) params.set('page', String(options.page));
      if (options?.limit) params.set('limit', String(options.limit));
      if (options?.familyId) params.set('familyId', options.familyId);
      ...
      if (options?.sortBy) params.set('sortBy', options.sortBy);
      if (options?.sortOrder) params.set('sortOrder', options.sortOrder);
      return request<{ events: any[]; total: number; page: number; limit: number; totalPages: number }>(`/timeline?${params.toString()}`);
    },
    ...
    bulkUpdateStatus: (ids: string[], status: string) =>
      request<{ updated: number }>('/timeline/bulk-update-status', { method: 'POST', body: { ids, status } }),
    ...
    byFamily: (familyId: string, options?: { page?: number; limit?: number; eventType?: string }) => {
      const params = new URLSearchParams();
      if (options?.page) params.set('page', String(options.page));
      if (options?.limit) params.set('limit', String(options.limit));
      if (options?.eventType) params.set('eventType', options.eventType);
      return request<{ events: any[]; total: number; page: number; limit: number; totalPages: number }>(`/timeline/family/${familyId}?${params.toString()}`);
    },
    byMember: (memberId: string, options?: { page?: number; limit?: number }) => {
      const params = new URLSearchParams();
      if (options?.page) params.set('page', String(options.page));
      if (options?.limit) params.set('limit', String(options.limit));
      return request<{ events: any[]; total: number; page: number; limit: number; totalPages: number }>(`/timeline/member/${memberId}?${params.toString()}`);
    },
    upcoming: () => request<{ events: any[] }>('/timeline/upcoming'),
    calendar: (year: number, month: number) => request<{ events: any[]; grouped: Record<string, any[]> }>(`/timeline/calendar?year=${year}&month=${month}`),
    calendarWeek: (startDate: string) => request<{ events: any[] }>(`/timeline/calendar/week?startDate=${startDate}`),
    agenda: (dateFrom: string, dateTo: string) => request<{ events: any[] }>(`/timeline/agenda?dateFrom=${dateFrom}&dateTo=${dateTo}`),
    ...
    widget: () => request<{ upcoming: any[]; today: any[]; birthdays: any[]; recent: any[] }>('/timeline/widget'),
    update: (id: string, data: any) => request<any>(`/timeline/${id}`, { method: 'PATCH', body: data }),
    ...
    rsvp: (id: string, rsvpStatus: string) => request<any>(`/timeline/${id}/rsvp`, { method: 'POST', body: { rsvpStatus } }),
    participants: (id: string) => request<{ participants: any[]; stats: any }>(`/timeline/${id}/participants`),
    ...
    search: (q: string, page?: number, limit?: number) => {
      ...
      return request<{ events: any[]; total: number; page: number; limit: number; totalPages: number }>(`/timeline/search?${params.toString()}`);
    },
    ...
    addComment: (eventId: string, content: string, parentId?: string) =>
      request<any>(`/timeline/${eventId}/comments`, { method: 'POST', body: { content, parentId } }),
    getComments: (eventId: string, page?: number, limit?: number) => {
      const params = new URLSearchParams();
      if (page) params.set('page', String(page));
      if (limit) params.set('limit', String(limit));
      return request<{ comments: any[]; total: number; page: number; limit: number; totalPages: number }>(`/timeline/${eventId}/comments?${params.toString()}`);
    },
    ...
    getReactions: (eventId: string) =>
      request<{ reactions: any[]; grouped: Record<string, any[]>; total: number }>(`/timeline/${eventId}/reactions`),
    ...
    getActivity: (eventId: string, page?: number, limit?: number) => {
      ...
      if (page) params.set('page', String(page));
      if (limit) params.set('limit', String(limit));
      return request<{ activities: any[]; total: number; page: number; limit: number; totalPages: number }>(`/timeline/${eventId}/activity?${params.toString()}`);
    },
    getHistory: (eventId: string, page?: number, limit?: number) => {
      const params = new URLSearchParams();
      if (page) params.set('page', String(page));
      if (limit) params.set('limit', String(limit));
      return request<{ history: any[]; total: number; page: number; limit: number; totalPages: number }>(`/timeline/${eventId}/history?${params.toString()}`);
    },
    ...
    reminders: {
      list: (eventId: string) =>
        request<{ reminders: any[] }>(`/timeline/${eventId}/reminders`),
    ...
    exportJson: (eventId: string) =>
      request<any>(`/timeline/${eventId}/export/json`),
    exportCsv: (eventId: string) =>
      request<string>(`/timeline/${eventId}/export/csv`),
    ...
    getPrintVersion: (eventId: string) =>
      request<any>(`/timeline/events/${eventId}/print`),
    generatePrint: (eventId: string, options?: any) =>
      request<any>(`/timeline/events/${eventId}/print/generate`, {
        method: 'POST',
        body: JSON.stringify(options || {}),
      }),
    exportEventJson: (eventId: string) =>
      request<any>(`/timeline/events/${eventId}/export/json`),
    getAttendance: (eventId: string) =>
      request<any>(`/timeline/events/${eventId}/attendance`),
  ...
  eventInvitations: {
    create: (data: any) => request('/event-invitations', { method: 'POST', body: JSON.stringify(data) }),
    ...
    respond: (id: string, status: string, message?: string) => request(`/event-invitations/${id}/respond`, { method: 'PATCH', body: JSON.stringify({ status, message }) }),
  },
  clans: {
    list: (params?: { page?: number; limit?: number; search?: string; status?: string; country?: string; verified?: boolean }) => {
      const q = new URLSearchParams();
      if (params?.page) q.set('page', String(params.page));
      if (params?.limit) q.set('limit', String(params.limit));
    ...
    search: (slug: string, query: string) => request(`/clans/${slug}/search?q=${encodeURIComponent(query)}`),
    ...
    update: (id: string, data: any) => request(`/clans/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
    remove: (id: string) => request(`/clans/${id}`, { method: 'DELETE' }),
    join: (clanId: string, familyId: string) => request(`/clans/${clanId}/join`, { method: 'POST', body: JSON.stringify({ familyId }) }),
    leave: (clanId: string, familyId: string) => request(`/clans/${clanId}/leave`, { method: 'POST', body: JSON.stringify({ familyId }) }),
  },
  communities: {
    list: (params?: { page?: number; limit?: number; search?: string; status?: string; country?: string; verified?: boolean }) => {
      const q = new URLSearchParams();
    ...
    update: (id: string, data: any) => request(`/communities/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
    remove: (id: string) => request(`/communities/${id}`, { method: 'DELETE' }),
    getAdmins: (id: string) => request(`/communities/${id}/admins`),
    addAdmin: (id: string, data: any) => request(`/communities/${id}/admins`, { method: 'POST', body: JSON.stringify(data) }),
    updateAdmin: (id: string, adminId: string, data: any) => request(`/communities/${id}/admins/${adminId}`, { method: 'PATCH', body: JSON.stringify(data) }),
    removeAdmin: (id: string, adminId: string) => request(`/communities/${id}/admins/${adminId}`, { method: 'DELETE' }),
    getRequests: (id: string) => request(`/communities/${id}/requests`),
    createRequest: (id: string, data?: any) => request(`/communities/${id}/requests`, { method: 'POST', body: JSON.stringify(data || {}) }),
    deleteRequest: (id: string, requestId: string) => request(`/communities/${id}/requests/${requestId}`, { method: 'DELETE' }),
    approveRequest: (id: string, requestId: string) => request(`/communities/${id}/requests/${requestId}/approve`, { method: 'PATCH' }),
    rejectRequest: (id: string, requestId: string) => request(`/communities/${id}/requests/${requestId}/reject`, { method: 'PATCH' }),
  ...
  subclans: {
    ...
    create: (clanId: string, data: any) => request(`/clans/${clanId}/subclans`, { method: 'POST', body: JSON.stringify(data) }),
    createDirect: (data: any) => request('/subclans', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, data: any) => request(`/subclans/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
    remove: (id: string) => request(`/subclans/${id}`, { method: 'DELETE' }),
    getTree: (id: string) => request(`/subclans/${id}/tree`),
    getBreadcrumbs: (id: string) => request<{ id: string; name: string; slug: string; type: string }[]>(`/subclans/${id}/breadcrumbs`),
  },
  clanRequests: {
    create: (clanId: string, data: any) => request(`/clans/${clanId}/requests`, { method: 'POST', body: JSON.stringify(data) }),
    list: (clanId: string, status?: string) => request(`/clans/${clanId}/requests${status ? `?status=${status}` : ''}`),
    stats: (clanId: string) => request(`/clans/${clanId}/requests/stats`),
    mine: () => request('/clan-requests/mine'),
    accept: (id: string, response?: string) => request(`/clan-requests/${id}/accept`, { method: 'PATCH', body: JSON.stringify({ response }) }),
    reject: (id: string, response?: string) => request(`/clan-requests/${id}/reject`, { method: 'PATCH', body: JSON.stringify({ response }) }),
  ...
  clanHistory: {
    ...
    create: (clanId: string, data: { section: string; content: string }) => request(`/clans/${clanId}/history`, { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, content: string) => request(`/clan-history/${id}`, { method: 'PATCH', body: JSON.stringify({ content }) }),
    ...
    approve: (id: string, note?: string) => request(`/clan-history/${id}/approve`, { method: 'PATCH', body: JSON.stringify({ note }) }),
    reject: (id: string, note?: string) => request(`/clan-history/${id}/reject`, { method: 'PATCH', body: JSON.stringify({ note }) }),
    requestChanges: (id: string, note: string) => request(`/clan-history/${id}/request-changes`, { method: 'PATCH', body: JSON.stringify({ note }) }),
  },
  communityHistory: {
    ...
    create: (data: any) => request('/community-history', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, data: any) => request(`/community-history/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  communityGallery: {
    ...
    create: (data: any) => request('/community-gallery', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, data: any) => request(`/community-gallery/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
    remove: (id: string) => request(`/community-gallery/${id}`, { method: 'DELETE' }),
  },
  communityDirectory: {
    listByCommunity: (communityId: string, params?: { role?: string; verified?: boolean; status?: string; search?: string }) => {
      const q = new URLSearchParams();
    ...
    join: (communityId: string) => request('/community-directory', { method: 'POST', body: JSON.stringify({ communityId }) }),
    get: (id: string) => request(`/community-directory/${id}`),
    update: (id: string, data: any) => request(`/community-directory/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  communityEvents: {
    listByCommunity: (communityId: string, params?: { type?: string; status?: string; upcoming?: boolean }) => {
      const q = new URLSearchParams();
      if (params?.type) q.set('type', params.type);
      if (params?.status) q.set('status', params.status);
    ...
    create: (data: any) => request('/community-events', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, data: any) => request(`/community-events/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
    remove: (id: string) => request(`/community-events/${id}`, { method: 'DELETE' }),
  },
  communityNews: {
    listByCommunity: (communityId: string, params?: { type?: string; status?: string; featured?: boolean }) => {
      const q = new URLSearchParams();
      if (params?.type) q.set('type', params.type);
      if (params?.status) q.set('status', params.status);
    ...
    create: (data: any) => request('/community-news', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, data: any) => request(`/community-news/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  communityDocuments: {
    ...
    create: (data: any) => request('/community-documents', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, data: any) => request(`/community-documents/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  communityLocations: {
    ...
    distribution: (communityId: string) => request(`/community-locations/community/${communityId}/distribution`),
    get: (id: string) => request(`/community-locations/${id}`),
    create: (data: any) => request('/community-locations', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, data: any) => request(`/community-locations/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
    remove: (id: string) => request(`/community-locations/${id}`, { method: 'DELETE' }),
  },
  followers: {
    followCommunity: (communityId: string) => request(`/followers/community/${communityId}`, { method: 'POST' }),
    unfollowCommunity: (communityId: string) => request(`/followers/community/${communityId}`, { method: 'DELETE' }),
    checkCommunity: (communityId: string) => request<{ isFollowing: boolean }>(`/followers/community/${communityId}/check`),
    communityCount: (communityId: string) => request<{ count: number }>(`/followers/community/${communityId}/count`),
    ...
    checkClan: (clanId: string) => request<{ isFollowing: boolean }>(`/followers/clan/${clanId}/check`),
  ...
  bookmarks: {
    ...
    create: (entityType: string, entityId: string) => request('/bookmarks', { method: 'POST', body: JSON.stringify({ entityType, entityId }) }),
    check: (entityType: string, entityId: string) => request<{ isBookmarked: boolean }>(`/bookmarks/check?entityType=${entityType}&entityId=${entityId}`),
    remove: (id: string) => request(`/bookmarks/${id}`, { method: 'DELETE' }),
  },
  reputation: {
    community: (communityId: string) => request(`/reputation/community/${communityId}`),
    calculateCommunity: (communityId: string) => request(`/reputation/community/${communityId}/calculate`, { method: 'POST' }),
    clan: (clanId: string) => request(`/reputation/clan/${clanId}`),
    calculateClan: (clanId: string) => request(`/reputation/clan/${clanId}/calculate`, { method: 'POST' }),
  ...
  knowledgeBase: {
    ...
    byEntity: (entityType: string, entityId: string) => request<any[]>(`/knowledge-base/entity/${entityType}/${entityId}`),
    get: (id: string) => request(`/knowledge-base/${id}`),
    create: (data: any) => request('/knowledge-base', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, data: any) => request(`/knowledge-base/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  featured: {
    ...
    trendingCommunities: (limit?: number) => request<any[]>(`/featured/communities/trending?limit=${limit || 10}`),
    trendingClans: (limit?: number) => request<any[]>(`/featured/clans/trending?limit=${limit || 10}`),
    relatedCommunities: (communityId: string, limit?: number) => request<any[]>(`/featured/communities/${communityId}/related?limit=${limit || 5}`),
    relatedClans: (clanId: string, limit?: number) => request<any[]>(`/featured/clans/${clanId}/related?limit=${limit || 5}`),
  ...
  aiInsights: {
    ...
    storeCommunity: (communityId: string, data: any) => request(`/ai-insights/community/${communityId}`, { method: 'POST', body: JSON.stringify(data) }),
    storeClan: (clanId: string, data: any) => request(`/ai-insights/clan/${clanId}`, { method: 'POST', body: JSON.stringify(data) }),
  },
  clanGallery: {
    ...
    update: (id: string, data: any) => request(`/clan-gallery/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
    remove: (id: string) => request(`/clan-gallery/${id}`, { method: 'DELETE' }),
  },
  clanDirectory: {
    listByClan: (clanId: string, params?: { role?: string; verified?: boolean; status?: string; search?: string }) => {
      const q = new URLSearchParams();
      if (params?.role) q.set('role', params.role);
      if (params?.verified !== undefined) q.set('verified', String(params.verified));
      if (params?.status) q.set('status', params.status);
    ...
    join: (clanId: string) => request('/clan-directory', { method: 'POST', body: JSON.stringify({ clanId }) }),
    get: (id: string) => request(`/clan-directory/${id}`),
    update: (id: string, data: any) => request(`/clan-directory/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  clanEvents: {
    ...
    update: (id: string, data: any) => request(`/clan-events/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  clanDocuments: {
    ...
    create: (data: any) => request('/clan-documents', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, data: any) => request(`/clan-documents/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  clanLocations: {
    ...
    create: (data: any) => request('/clan-locations', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, data: any) => request(`/clan-locations/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  documentVault: {
    ...
    create: (data: any) => request('/document-vault', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, data: any) => request(`/document-vault/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
    remove: (id: string) => request(`/document-vault/${id}`, { method: 'DELETE' }),
    restore: (id: string) => request(`/document-vault/${id}/restore`, { method: 'PATCH' }),
    permanentDelete: (id: string) => request(`/document-vault/${id}/permanent`, { method: 'DELETE' }),
    ...
    versions: {
      ...
      create: (docId: string, data: any) => request(`/document-vault/${docId}/versions`, { method: 'POST', body: JSON.stringify(data) }),
      restore: (docId: string, version: number) => request(`/document-vault/${docId}/versions/${version}/restore`, { method: 'PATCH' }),
    },
    folders: {
      list: (params?: Record<string, string>) => {
        const q = new URLSearchParams();
      ...
      create: (data: any) => request('/document-vault/folders', { method: 'POST', body: JSON.stringify(data) }),
      update: (id: string, data: any) => request(`/document-vault/folders/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
      remove: (id: string) => request(`/document-vault/folders/${id}`, { method: 'DELETE' }),
    },
    shares: {
      list: (docId: string) => request(`/document-vault/${docId}/shares`),
      create: (data: any) => request('/document-vault/shares', { method: 'POST', body: JSON.stringify(data) }),
      revoke: (shareId: string) => request(`/document-vault/shares/${shareId}`, { method: 'DELETE' }),
    },
    ...
    collections: {
      ...
      create: (data: any) => request('/document-vault/collections', { method: 'POST', body: JSON.stringify(data) }),
      update: (id: string, data: any) => request(`/document-vault/collections/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
      remove: (id: string) => request(`/document-vault/collections/${id}`, { method: 'DELETE' }),
      addItem: (id: string, data: any) => request(`/document-vault/collections/${id}/items`, { method: 'POST', body: JSON.stringify(data) }),
      removeItem: (collectionId: string, itemId: string) => request(`/document-vault/collections/${collectionId}/items/${itemId}`, { method: 'DELETE' }),
    },
    attachments: {
      create: (data: any) => request('/document-vault/attachments', { method: 'POST', body: JSON.stringify(data) }),
      getByEntity: (entityType: string, entityId: string) => request(`/document-vault/attachments/${entityType}/${entityId}`),
      remove: (id: string) => request(`/document-vault/attachments/${id}`, { method: 'DELETE' }),
    },
    verifications: {
      create: (data: any) => request('/document-vault/verifications', { method: 'POST', body: JSON.stringify(data) }),
    ...
    gallery: {
      ...
      create: (data: any) => request('/document-vault/gallery', { method: 'POST', body: JSON.stringify(data) }),
      update: (id: string, data: any) => request(`/document-vault/gallery/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
    ...
    references: {
      ...
      create: (data: any) => request('/document-vault/references', { method: 'POST', body: JSON.stringify(data) }),
      update: (id: string, data: any) => request(`/document-vault/references/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
      remove: (id: string) => request(`/document-vault/references/${id}`, { method: 'DELETE' }),
    },
    publicPages: {
      mine: () => request('/document-vault/public-pages/mine'),
      create: (data: any) => request('/document-vault/public-pages', { method: 'POST', body: JSON.stringify(data) }),
      update: (id: string, data: any) => request(`/document-vault/public-pages/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
    ...
    knowledgeBase: {
      ...
      create: (data: any) => request('/document-vault/knowledge-base', { method: 'POST', body: JSON.stringify(data) }),
      update: (id: string, data: any) => request(`/document-vault/knowledge-base/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
      remove: (id: string) => request(`/document-vault/knowledge-base/${id}`, { method: 'DELETE' }),
      vote: (id: string, helpful: boolean) => request(`/document-vault/knowledge-base/${id}/vote`, { method: 'POST', body: JSON.stringify({ helpful }) }),
    },
    analytics: {
      ...
      mostViewed: (limit?: number) => request(`/document-vault/analytics/most-viewed?limit=${limit || 10}`),
    ...
    timeline: {
      ...
      community: (communityId: string) => request(`/document-vault/timeline/community/${communityId}`),
  ...
  tree: {
    family: (familyId: string, depth?: number) => request(`/tree/family/${familyId}?depth=${depth || 10}`),
    clan: (clanId: string, depth?: number) => request(`/tree/clan/${clanId}?depth=${depth || 5}`),
    community: (communityId: string, depth?: number) => request(`/tree/community/${communityId}?depth=${depth || 4}`),
    ancestors: (memberId: string, depth?: number) => request(`/tree/member/${memberId}/ancestors?depth=${depth || 10}`),
    descendants: (memberId: string, depth?: number) => request(`/tree/member/${memberId}/descendants?depth=${depth || 10}`),
    ...
    stats: (entityType: string, entityId: string) => request(`/tree/stats/${entityType}/${entityId}`),
    enhancedStats: (entityType: string, entityId: string) => request(`/tree/enhanced-stats/${entityType}/${entityId}`),
    expandNode: (entityType: string, entityId: string, depth?: number) =>
      request('/tree/node/expand', { method: 'POST', body: JSON.stringify({ entityType, entityId, depth }) }),
    commonAncestor: (memberIdA: string, memberIdB: string, depth?: number) =>
      request(`/tree/common-ancestor?memberIdA=${memberIdA}&memberIdB=${memberIdB}&depth=${depth || 20}`, { method: 'POST' }),
    relationshipPath: (memberIdA: string, memberIdB: string, depth?: number) =>
      request(`/tree/relationship-path?memberIdA=${memberIdA}&memberIdB=${memberIdB}&depth=${depth || 20}`, { method: 'POST' }),
    ...
    views: {
      list: (page?: number, limit?: number) => request(`/tree/views?page=${page || 1}&limit=${limit || 20}`),
      get: (id: string) => request(`/tree/views/${id}`),
      create: (data: any) => request('/tree/views', { method: 'POST', body: JSON.stringify(data) }),
      update: (id: string, data: any) => request(`/tree/views/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
      remove: (id: string) => request(`/tree/views/${id}`, { method: 'DELETE' }),
      public: (page?: number, limit?: number) => request(`/tree/views/public?page=${page || 1}&limit=${limit || 20}`),
      publicById: (id: string) => request(`/tree/views/public/${id}`),
    },
    layoutCache: {
      get: (entityType: string, entityId: string, treeType: string, layout: string) =>
        request(`/tree/layout-cache?entityType=${entityType}&entityId=${entityId}&treeType=${treeType}&layout=${layout}`),
      save: (data: any) => request('/tree/layout-cache', { method: 'POST', body: JSON.stringify(data) }),
    },
    bookmarks: {
      list: (entityType?: string) => request(`/tree/bookmarks${entityType ? `?entityType=${entityType}` : ''}`),
      create: (data: any) => request('/tree/bookmarks', { method: 'POST', body: JSON.stringify(data) }),
      remove: (id: string) => request(`/tree/bookmarks/${id}`, { method: 'DELETE' }),
    },
    searchHistory: {
      list: (limit?: number) => request(`/tree/search-history?limit=${limit || 20}`),
      create: (data: any) => request('/tree/search-history', { method: 'POST', body: JSON.stringify(data) }),
