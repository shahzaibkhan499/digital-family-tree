...
export default function ClanGalleryPage() {
  ...
  useEffect(() => {
    ...
    api.clanGallery.listByClan(clan.id, p)
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!clan || !createForm.title.trim() || !createForm.url.trim()) return;
    setCreating(true);
    try {
      await api.clanGallery.create({
        clanId: clan.id,
      ...
      const data = await api.clanGallery.listByClan(clan.id, activeType !== 'All' ? { type: activeType } : undefined);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-slate-900 dark:text-white">Clan Gallery</h1>
        </div>
        <button
