...
const ROLE_FILTERS = ['All', 'Member', 'Leader', 'Historian', 'Elder', 'Representative'] as const;
...
const ROLE_COLORS: Record<string, string> = {
  ...
  Historian: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400',
  Elder: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400',
  Representative: 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400',
};
...
export default function ClanMembersPage() {
  ...
  useEffect(() => {
    ...
    const cid = clan.id;
    Promise.allSettled([
      api.clanDirectory.listByClan(cid, {
        role: activeRole !== 'All' ? activeRole : undefined,
        verified: verifiedFilter,
        search: searchQuery || undefined,
      }),
      api.clanDirectory.stats(cid),
  ...
  }, [clan, activeRole, verifiedFilter, searchQuery]);

  const handleJoin = async () => {
    if (!clan) return;
    setJoining(true);
    try {
      await api.clanDirectory.join(clan.id);
      setIsMember(true);
      const s = await api.clanDirectory.stats(clan.id).catch(() => null);
      setStats(s);
      const data = await api.clanDirectory.listByClan(clan.id, {
        role: activeRole !== 'All' ? activeRole : undefined,
        verified: verifiedFilter,
        search: searchQuery || undefined,
      });
    ...
    <div className="space-y-6">
      ...
      {loadingMembers ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-300 bg-white p-12 text-center dark:border-slate-700 dark:bg-slate-900">
          ...
          <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">Be the first to join this clan.</p>
        </div>
      ) : (
