...
const TYPE_COLORS: Record<string, string> = {
  ...
  Gathering: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400',
...
export default function CommunityEventsPage() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      api.communityEvents.listByCommunity(cid, activeType !== 'All' ? { type: activeType } : undefined),
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!community || !createForm.title.trim() || !createForm.startDate) return;
    setCreating(true);
    try {
      await api.communityEvents.create({
      ...
      const data = await api.communityEvents.listByCommunity(community.id, activeType !== 'All' ? { type: activeType } : undefined);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-slate-900 dark:text-white">Community Events</h1>
        </div>
        <button
      ...
      {loadingEvents ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-300 bg-white p-12 text-center dark:border-slate-700 dark:bg-slate-900">
          ...
          <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">Create the first event for this community.</p>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ${TYPE_COLORS[event.type] || TYPE_COLORS.Other}`}>
                      {event.type}
                    </span>
                    {event.status && (
