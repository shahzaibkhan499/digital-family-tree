...
const DOC_TYPES = ['Research', 'Record', 'Genealogy', 'Historical', 'Legal', 'Other'] as const;

const TYPE_COLORS: Record<string, string> = {
  Research: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
  Record: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400',
  Genealogy: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400',
...
export default function CommunityDocumentsPage() {
  ...
  useEffect(() => {
    if (!community) return;
    setLoadingDocs(true);
    api.communityDocuments.listByCommunity(community.id, activeType !== 'All' ? { type: activeType } : undefined)
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!community || !createForm.title.trim()) return;
    setCreating(true);
    try {
      await api.communityDocuments.create({
      ...
      const data = await api.communityDocuments.listByCommunity(community.id, activeType !== 'All' ? { type: activeType } : undefined);
      setDocuments(Array.isArray(data) ? data : []);
      setShowCreate(false);
      setCreateForm({ type: 'Research', title: '', description: '', url: '', source: '' });
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ${TYPE_COLORS[doc.type] || TYPE_COLORS.Other}`}>
                      {doc.type}
                    </span>
