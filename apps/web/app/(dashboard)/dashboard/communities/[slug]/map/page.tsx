...
export default function CommunityMapPage() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      api.communityLocations.listByCommunity(cid, activeType !== 'All' ? { type: activeType } : undefined),
      api.communityLocations.distribution(cid),
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!community || !createForm.name.trim() || !createForm.country.trim()) return;
    setCreating(true);
    try {
      await api.communityLocations.create({
      ...
      const data = await api.communityLocations.listByCommunity(community.id, activeType !== 'All' ? { type: activeType } : undefined);
      setLocations(Array.isArray(data) ? data : []);
      setShowCreate(false);
      setCreateForm({ type: 'Current', name: '', country: '', region: '', latitude: '', longitude: '', description: '' });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-slate-900 dark:text-white">Community Locations</h1>
        </div>
        <button
      ...
      {loadingLocations ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-300 bg-white p-12 text-center dark:border-slate-700 dark:bg-slate-900">
          ...
          <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">Map the geographic distribution of your community.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {locations.map((loc: any) => (
            <div key={loc.id} className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-base font-semibold text-slate-900 dark:text-white">{loc.name}</h3>
              ...
              {loc.description && (
                <p className="mt-3 text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{loc.description}</p>
              )}
            </div>
          ))}
        </div>
